...
public class QCircleTemplate {
    ...
    protected Callable<Intent> mAsyncCreator = null;
    protected boolean mShouldSetLayoutParams = true;
    ...
    /**
     * Create a template for dialog.
     *
     * @param context Activity which has the template.
     */
    /* package */ QCircleTemplate(Context context, TemplateType type, boolean shouldSetLayoutParams) {
        mContext = context;
        mShouldSetLayoutParams = shouldSetLayoutParams;
        if (type == null)
     ...
     */
    @Deprecated
    public void setFullscreenIntent(final IntentCreatorAsync creatorAsync) {
        mFullscreenIntent = null;
        mAsyncCreator = new Callable<Intent>() {
            @Override
            public Intent call() throws Exception {
                return creatorAsync.getIntent();
            }
        };
    }
     ...
     */
    @Deprecated
    public void setFullscreenIntent(Callable<Intent> creatorAsync) {
        mFullscreenIntent = null;
    ...
    private boolean initCircleLayoutParam() {
        ...
        int mYpos = QCircleFeature.getYPosition(mContext);
                    ...
                    if (ACTION_ACCESSORY_COVER_EVENT.equals(action)) {
                        ...
                        if (quickCoverState == EXTRA_ACCESSORY_COVER_CLOSED) { // closed
                            ...
                            if (mFullscreenIntent != null && mContext != null) {
                                ...
                                try {
                                    Intent launching = mAsyncCreator.call();
                                    if (launching != null) {
                                        try {
    ...
    public int getDiameter() {
        if (mFullSize == 0) {
            if (!initCircleLayoutParam()) return -1;
        }
        return mFullSize;
    }
